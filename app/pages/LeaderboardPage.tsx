...
import type { NetworkSlug, LeaderboardPeriod, LeaderboardSort, LeaderboardResponse } from '@/types/api';
...
export function resolveNetworkParam(raw: string): NetworkSlug {
  if (isNetworkSlug(raw)) return raw;
  const mapped = TICKER_TO_SLUG[raw.toLowerCase()];
  return mapped ?? 'solana';
}

export function isPeriod(s: string): s is LeaderboardPeriod {
...
export default function LeaderboardPage({ initialLeaderboard }: { initialLeaderboard?: LeaderboardResponse | null } = {}) {
  ...
  const { validators, generatedAt, loading, loadingMore, error, hasMore, loadMore } = useLeaderboard(network, period, sort, initialLeaderboard);
