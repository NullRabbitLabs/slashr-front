...
import { feedWindow, mapEvents, type ApiEvent, type FeedItem } from './feedIncidents';

/// Fetch the feed window: everything in the last FEED_WINDOW_DAYS, capped at
/// FEED_MAX_ITEMS. `now` is injectable so the caller can pin it in a test.
export async function fetchFeedItems(now: Date = new Date()): Promise<FeedItem[]> {
  const { from, limit } = feedWindow(now);
  const url = `${apiBase()}/v1/events?limit=${limit}&from=${encodeURIComponent(from)}`;
