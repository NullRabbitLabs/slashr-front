...
export const EVENT_TYPE_LABELS: Record<EventType, string> = {
  ...
  dot_offence:         'Offence reported on-chain. Any slash is deferred and may be zero.',
  ...
  tia_jailed_downtime:     'Offline too long on Celestia. Jailed, no stake deducted.',
...
export const EVENT_TYPE_DESCRIPTIONS: Record<EventType, string> = {
  ...
  dot_offence:         'Polkadot reported an offence against this validator, meaning the chain recorded misbehaviour such as being unresponsive or signing conflicting blocks. This is a report, not a completed penalty. Polkadot defers slashing for about 28 days and governance can cancel a slash before it applies, and many offences carry no stake penalty at all: unresponsiveness is only slashed when more than 10% of validators are offline at once, so the reported fraction is frequently zero. Where a fraction was reported, it is shown on the event. Stake actually being reduced is recorded separately as a slashing event.',
  ...
  tia_jailed_downtime:     'Validator missed too many blocks on Celestia and was jailed. Celestia sets its downtime slash fraction to zero, so nothing is deducted from stake: the penalty is the jailing itself, and the chain records the deduction as zero. The validator must manually unjail before rejoining consensus, and delegators earn no rewards until it does.',
